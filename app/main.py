...
app.include_router(ui.search.router, include_in_schema=False)
...
# Exception handlers
async def _route_error_response(
    request: Request,
    error_title: str,
    error_message: str,
    status_code: int,
...
async def validation_exception_handler(
    ...
    if (
        request.url.path.startswith("/get/")
        and "download" in request.query_params
        and request.query_params["download"] == ""
    ):
    ...
    if request.url.path.startswith(("/get/", "/download/")):
        error_message = (
            exc.errors()[0]["msg"]
            if exc.errors()
            else "An unknown error occurred while processing your request."
        )
        return await _route_error_response(
            request,
...
async def image_invalid_exception_handler(
    request: Request,
    exc: ImageInvalidError,
) -> Response:
